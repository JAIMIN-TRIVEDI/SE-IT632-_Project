...
import { approveRefund } from '../services/messService'
...
const STUDENT_STATUS_OPTIONS = ['all', 'active', 'expired', 'cancelled', 'none']
const SUBSCRIPTION_STATUS_OPTIONS = ['all', 'active', 'expired', 'cancelled', 'refund_pending', 'refund_approved']
const PAYMENT_STATUS_OPTIONS = ['all', 'success', 'pending', 'failed', 'refunded']
...
function MessRecords() {
  ...
  const { search, setSearch, debouncedSearch, isDebouncing } = useSearch('', 300)

  const studentsHook = useStudents({ enabled: activeTab === 'students', search: debouncedSearch })
  const subscriptionsHook = useSubscriptions({ enabled: activeTab === 'subscriptions', search: debouncedSearch })
  const paymentsHook = usePayments({ enabled: activeTab === 'payments', search: debouncedSearch })
    ...
    () => [
      {
        ...
        render: (row) => (
          <Box>
            <Typography fontWeight={600}>{row.studentId?.name || 'Unknown'}</Typography>
      ...
      },
      {
        ...
        label: 'Refund',
        render: (row) => {
          if (!row.refund?.requested) return 'None'
          const state = row.refund?.approved ? 'Approved' : 'Pending'
          return `₹${row.refund?.amount ?? 0} (${state})`
        },
      },
      {
        key: 'action',
        ...
        render: (row) => (
          row.refund?.requested && !row.refund?.approved ? (
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  setActionError(err.response?.data?.message || err.message || 'Failed to approve refund.')
                } finally {
                  setApprovingId('')
                }
              }}
            ...
            >
              {approvingId === row._id ? 'Approving...' : 'Approve Refund'}
            </Button>
          ) : (
            <Typography fontSize={12} color="text.secondary">No action</Typography>
          )
        ),
      },
    ],
  ...
  const statsCards =
    activeTab === 'students'
      ...
      : activeTab === 'subscriptions'
        ? [
            { label: 'Total Subscriptions', value: subscriptionsHook.stats.total },
        ...
        : [
            ...
            { label: 'Total Revenue', value: `₹${paymentsHook.stats.totalRevenue}` },
          ]
        ...
        {activeTab === 'payments' && (
          <>
            <TextField
              ...
              onChange={(event) => paymentsHook.setStartDate(event.target.value)}

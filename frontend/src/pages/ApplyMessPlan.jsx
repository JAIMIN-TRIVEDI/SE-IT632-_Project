    ...
    >
      ...
      >
        <Box display="flex" alignItems="center" gap={1} mb={1}>
          <RestaurantMenuIcon
            sx={{
              color: isActive || isCurrent ? 'rgba(255,255,255,0.9)' : 'primary.main',
              fontSize: 20,
            }}
          />
          <Typography
        ...
        <Box display="flex" alignItems="baseline" gap={0.5}>
          ...
          <Typography
            fontSize={13}
            color={isActive || isCurrent ? 'rgba(255,255,255,0.75)' : 'text.secondary'}
          >
        ...
        <Box display="flex" alignItems="center" gap={0.5} mt={1}>
          <CalendarMonthIcon
            sx={{
              fontSize: 14,
              color: isActive || isCurrent ? 'rgba(255,255,255,0.7)' : 'text.secondary',
            }}
          />
          <Typography
            fontSize={12}
          ...
          {features.map((f, i) => (
            <Box key={i} display="flex" alignItems="flex-start" gap={1}>
              {f.included ? (
                ...
                <CancelIcon sx={{ fontSize: 16, color: 'text.disabled', mt: 0.2, flexShrink: 0 }} />
              )}
              <Typography
                fontSize={13}
          ...
          <Button
            ...
            sx={{
              ...
              ...(isActive && {
                ...
                '&:hover': {
                  background: 'linear-gradient(135deg, #1e4fdb 0%, #1a369a 100%)',
                },
              }),
            }}
          >
...
function SubscriptionStatusCard({ subscription, currentStatus }) {
  ...
  const statusMap = {
    ...
    cancelled: {
      label: 'Cancelled',
      color: 'error',
      message: 'Your plan has been cancelled. You can now purchase a new mess plan.',
    },
    expired: {
      label: 'Expired',
      color: 'default',
      message: 'Your previous plan has expired. You can purchase a new mess plan.',
    },
  }
  ...
  return (
    ...
    >
      <Box display="flex" alignItems="center" gap={1} mb={0.75}>
        ...
        <Chip label={current.label} color={current.color} size="small" sx={{ fontWeight: 700 }} />
      </Box>
...
function ActiveSubscriptionBanner({ subscription, onCancel, cancelling }) {
  const cancellationRequested = Boolean(subscription.refund?.requested && !subscription.refund?.approved)
  ...
  return (
    ...
    >
      <Box display="flex" alignItems="flex-start" justifyContent="space-between" flexWrap="wrap" gap={2}>
        <Box>
          <Box display="flex" alignItems="center" gap={1} mb={1}>
            ...
            <Typography fontWeight={700} fontSize={16} color={cancellationRequested ? 'warning.main' : 'success.main'}>
              Active Subscription
            </Typography>
          </Box>
          ...
          <Typography fontSize={13} color="text.secondary" mt={0.5}>
            Valid until{' '}
            <strong>{new Date(subscription.endDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</strong>
          </Typography>
        </Box>
        <Box textAlign="right">
          <Typography fontSize={28} fontWeight={900} color={daysLeft <= 5 ? 'error.main' : 'text.primary'}>
            {daysLeft}
          </Typography>
      ...
      >
        {cancellationRequested ? 'Cancellation Requested' : cancelling ? 'Cancelling...' : 'Cancel Subscription'}
      </Button>
      {cancellationRequested && (
        <Typography color="warning.main" mt={1} fontSize={13}>
          Cancellation requested. Refund amount pending approval: ₹{subscription.refund?.amount}
        </Typography>
      )}
    </Card>
  )
...
export default function ApplyMessPlan() {
  ...
  const handleSelectPlan = async (plan) => {
    if (!canPurchaseNewPlan) {
      notify('You already have an active plan. New purchase is allowed after full cancellation/expiry.', 'warning')
      return
    }
  ...
  const handleCancel = async () => {
    if (!window.confirm('Cancel subscription? Refund will be calculated.')) return

    setCancelling(true)
    try {
      const res = await api.post('/mess/subscription/cancel')
      notify(`Cancellation requested. Refund amount: ₹${res.data.refundAmount}`, 'info')
      await refreshSubscription()
    } catch (err) {
  ...
  if (loadingPlans) {
    return (
      <Box display="flex" alignItems="center" justifyContent="center" minHeight="60vh" flexDirection="column" gap={2}>
        <CircularProgress />
        <Typography color="text.secondary" fontSize={14}>Loading mess plans…</Typography>
      </Box>
    )
  }
    ...
    <Box sx={{ maxWidth: 1100, mx: 'auto' }}>
      ...
      <Box mb={4}>
        ...
        <Typography color="text.secondary" fontSize={14}>
          Choose a plan and pay securely via Razorpay. Your subscription activates instantly after payment.
        </Typography>
      </Box>
      ...
      {/* Plan grid */}
      {plans.length === 0 ? (
        <Card sx={{ p: 5, textAlign: 'center', border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
        ...
        >
          {plans.map((plan) => {
            const isCurrentPlan =
              hasActiveAccess &&
              (subscription?.planId?._id === plan._id || subscription?.planId === plan._id)
            return (
              <PlanCard
                key={plan._id}
                plan={plan}
      ...
      {/* Security note */}
      <Divider sx={{ mb: 3 }} />
      <Box display="flex" alignItems="center" gap={1.5} justifyContent="center" flexWrap="wrap">
        <LockIcon sx={{ fontSize: 16, color: 'text.disabled' }} />
        <Typography fontSize={12} color="text.secondary">
          Payments are processed securely by{' '}
          <strong style={{ color: '#526cf5' }}>Razorpay</strong>. We do not store your card details.
        </Typography>

...
import { fetchSubscriptionsRecords } from '../services/messRecordsService'

export function useSubscriptions({ enabled, search }) {
  ...
  const fetchData = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      setLoading(true)
      setError('')
      const response = await fetchSubscriptionsRecords(queryParams)
